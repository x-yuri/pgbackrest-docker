...
docker run --rm -v $PWD:/app alpine:3.15 sh -euc '
    { . app/.env
    . app/standby/.env
    ...
    rm /etc/ssh/ssh_host*
    mkdir host-keys-standby
    ssh-keygen -A
    cp /etc/ssh/ssh_host* host-keys-standby
    ...
    awk -v host=db "{print host, \$1, \$2}" \
        ...
        host-keys/ssh_host_rsa_key.pub \
        > known_hosts-backups
    awk -v "host=[$STANDBY_IP]:$STANDBY_PORT" "{print host, \$1, \$2}" \
        host-keys-standby/ssh_host_ecdsa_key.pub \
        host-keys-standby/ssh_host_ed25519_key.pub \
        host-keys-standby/ssh_host_rsa_key.pub \
        >> known_hosts-backups

    awk -v "host=[$BACKUPS_IP]:$BACKUPS_PORT" "{print host, \$1, \$2}" \
        host-keys/ssh_host_ecdsa_key.pub \
        host-keys/ssh_host_ed25519_key.pub \
        host-keys/ssh_host_rsa_key.pub \
        > known_hosts-standby
